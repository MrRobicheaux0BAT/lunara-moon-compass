...
app.innerHTML = `
  ...
  <main class="stage">
    ...
    <section class="compass-wrap" id="compassWrap" aria-label="Moon compass">
      ...
      <div class="compass" id="compass">
        ...
        <div class="moon-orbit" id="moonOrbit">
          <div class="moon-marker" id="moonMarker">
            <div class="disc" id="moonDisc"></div>
          </div>
        </div>
        <div class="hub"></div>
      </div>
  ...
  <div class="overlay" id="overlay">
    <div class="sheet">
      ...
      <p id="overlayCopy">
        Needs location and compass. After you allow access, hold the phone flat.
      </p>
...
function describeVisibility({ altitude, lit, sunAltitude, name }) {
  if (altitude < -1) {
    return {
...
function paintMeta() {
  ...
  if (!base) return
  if (state.smoothHeading == null || state.moonAzimuth == null) {
    metaEl.innerHTML = base
    return
  }
  metaEl.innerHTML = `${base}<br>Turn <strong>${formatTurn(state.moonAzimuth - state.smoothHeading)}</strong> to face it`
}
...
function paintPhaseDisc(phase, fraction) {
  ...
  moonDisc.style.setProperty('--phase-x', `${side * (15 + shade * 70)}%`)
  moonDisc.style.setProperty('--phase-shade', String(0.2 + shade * 0.75))
  moonDisc.style.setProperty('--lit', String(lit))
}
...
function updateMoon() {
  ...
  const times = getMoonTimes(now, state.lat, state.lng)

  state.moonAzimuth = normalizeAngle(pos.azimuth)
  state.moonAltitude = pos.altitude

  const phaseInfo = describePhase(illum.phase, illum.fraction)
  const visibility = describeVisibility({
    altitude: state.moonAltitude,
    lit: phaseInfo.lit,
  ...
  altitudeEl.textContent = below
    ? `Below horizon · ${Math.abs(state.moonAltitude).toFixed(0)}°`
    : `Look up ${state.moonAltitude.toFixed(0)}°`

  metaEl.dataset.base = `Moon bearing <strong>${formatBearing(state.moonAzimuth)}</strong>`
  paintMeta()

  phaseLine.innerHTML = `<strong>${phaseInfo.name}</strong> · ${phaseInfo.lit}% illuminated`
  visibilityLine.textContent = visibility.summary

  const rise = times.rise ? formatClock(times.rise) : times.alwaysUp ? 'always up' : 'none today'
  const set = times.set ? formatClock(times.set) : times.alwaysDown ? 'always down' : 'none today'
  timingLine.textContent = `Moonrise ${rise} · Moonset ${set} · Day ${phaseInfo.ageDays.toFixed(1)} of ~29.5`
...
function renderLoop(now) {
  ...
  if (state.moonAzimuth != null && moonOrbit) {
    moonOrbit.style.setProperty('--moon-a', `${state.moonAzimuth}deg`)
  }
